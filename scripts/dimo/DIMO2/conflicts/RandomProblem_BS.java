...
public class RandomProblem_BS extends Problem_BS {
	 ...
	 */
	public RandomProblem_BS(int k, RandomElement re) {
		// init RandomElement:
		this.re = re;				
		if (k < 1) {
			this.os_dim=1;
	...
	public double[] getFitness(boolean[] decisionVector) {
		...
		for (int i=0;i<os_dim;i++) {
			objectiveVector[i] = re.uniform(0,1);        // uniform number out of [0,1]
		}
