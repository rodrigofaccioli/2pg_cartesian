...
int main(int argc, char *argv[]){
	...
	display_msg("Sort by nsga2 - Reading the configure file \n");
	...
	nsga2_t *nsga2 = NULL; // main of nsga2
...
/**************** START GETTING FRONT *************************/
    ...
    //Setting dominancep and nsga2
	dominancep = allocate_dominancep(&num_files);
	nsga2 = allocate_nsga2(&num_files);
...
/*
	...
	solution_p[6].obj_values[0] = 2;
	solution_p[6].obj_values[1] = 3;
	solution_p[7].obj_values[0] = 2;
	solution_p[7].obj_values[1] = 3;
	solution_p[8].obj_values[0] = 2;
	solution_p[8].obj_values[1] = 3;
	solution_p[9].obj_values[0] = 2;
	solution_p[9].obj_values[1] = 3;
*/

	set_dominancep(dominancep, solution_p, &num_files);
	//printf("show_dominance\n");
	//show_dominancep(dominancep, &num_files);
	//printf("set_front from nsga2\n\n");
	
	initialize_nsga2(nsga2, dominancep, &num_files, file_names);

	set_front(nsga2, dominancep, &num_files);

	//printf("set_crowding_distance from nsga2\n\n");
	////set_crowding_distance(nsga2, &num_files);
	//printf("order from nsga2\n\n");
//	clock_t tic = clock();
	order(nsga2, &num_files);

//	clock_t toc = clock();
//	printf("Elapsed: %f seconds\n", (double)(toc - tic) / CLOCKS_PER_SEC);

	save_nsga2_v1(nsga2, &num_files);
        save_nsga2_v2(nsga2, &num_files);

	

	desallocate_dominancep(dominancep, &num_files);
	desallocate_nsga2(nsga2);
